...
export default function CongesMembre({item}) { 
    const navigation = useNavigation();
    const verif = () => {
        if(item.etat == 0){
            return <TouchableOpacity style={styles.card} onPress={()=>{navigation.navigate('TraitementConge',{id_conge: item.id , id_employe : item.id_employe , etat_conge : item.etat} )}}>
