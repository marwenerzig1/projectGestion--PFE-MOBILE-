...
export default function EmploiDeTempsItems({item , language}) { 
        ...
        const test=() => {
            if(item.net_heures == "00:00:00"){
                return <View style={styles.card}>
                            <Text style={{fontWeight:'bold'}}>{item.nom.toUpperCase()} {item.prenom.toUpperCase()}</Text>
                            <Text style={styles.text12} >{item.date}</Text> 
                            <IconButton icon="map" color={'#DFDE07'} size={39} style={{ marginLeft:245 , marginTop: -47 }} onPress={()=> navigation.navigate('Location',{id_pointage : item.id})} />
