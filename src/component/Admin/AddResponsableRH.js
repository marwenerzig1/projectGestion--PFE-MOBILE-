...
export default function AddResponsableRH({item}) { 
  const navigation = useNavigation();
  const [loading, setLoading] = useState(false); 
  const {ip_config} = useSelector(state => state.userReducer);  

  const pressAjouter = (id) => {
    var InsertAPIURL="http://"+ip_config+"/api/AjouterResponsableRH";
    ...
    fetch(InsertAPIURL,
      ...
      .then((resData) =>{
        if (JSON.stringify(resData[0]['code']).toString() === '"201"' ) {
            setLoading(true);
            setLoading(false);
            navigation.navigate('ResponsableRHAdmin') ; 
    ...
    return (
         <TouchableOpacity style={styles.list} onPress={()=>{Alert.alert(
          ...
          '' , 
          //body
          'Est-ce que tu veux ajouter '+ item.nom+' '+item.prenom +' comme Responsable RH ? ',
          [
            {
              text: 'Ajouter',
              onPress: () => { 
