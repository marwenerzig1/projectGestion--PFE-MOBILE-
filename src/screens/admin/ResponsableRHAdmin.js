...
export default function ResponsableRHAdmin() {

  const navigation = useNavigation();  
  const {ip_config,etat_rh} = useSelector(state => state.userReducer);  
  ...
  const AffichageButton = () => {
    if(etat_rh == 0 ){
      ...
      style={{
        ...
        position: 'absolute',
        bottom: 15,
        backgroundColor: '#51AE35'
      }}
      onPress={() => navigation.navigate('AjouterCompteResponsableRH',{fonction2: "Responsable RH"})}
  /> ; 
    }
  }

  const getResponsable = () => {
    fetch("http://"+ip_config+"/api/getResponsableRH")
